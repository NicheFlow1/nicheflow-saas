...
import { LayoutDashboard, Zap, Search, Radio, Bot, BarChart2, TrendingUp, Settings, Flame, Eye, Star, Users, Sparkles } from 'lucide-react';
...
const NAV_SECTIONS = [
  {
    ...
    items: [
      ...
      { href: '/daily-picks', label: 'Daily Picks', icon: Star, badge: 'NEW' },
  ...
  },
  {
    ...
    items: [
      ...
      { href: '/audience', label: 'Audience Intel', icon: Users, badge: 'NEW' },
  ...
  },
  {
    label: "Build",
    items: [
      { href: '/dashboard/starter', label: 'Starter Kit Builder', icon: Sparkles },
      ...
      <nav style={{ flex:1,padding:'8px 8px' }}>
        {NAV_SECTIONS.map(section => (
          <div key={section.label} style={{ marginBottom: 4 }}>
            <div style={{ fontSize:10,fontWeight:700,color:'#3d4160',letterSpacing:'0.1em',textTransform:'uppercase',padding:'10px 8px 4px' }}>{section.label}</div>
            {section.items.map(({ href, label, icon: Icon, badge }: any) => {
              const active = pathname === href || (href !== '/dashboard' && pathname?.startsWith(href));
              return (
                <Link key={href} href={href} style={{ display:'flex',alignItems:'center',gap:10,padding:'7px 10px',borderRadius:8,marginBottom:1,textDecoration:'none',background:active?'rgba(99,102,241,0.12)':'transparent',color:active?'#a5b4fc':'#6b7194',fontWeight:active?600:400,fontSize:13,transition:'all 0.15s' }}>
                  <Icon size={14} style={{ flexShrink:0,color:active?'#818cf8':'#4b5280' }}/>
                  <span style={{ flex:1 }}>{label}</span>
                  {badge && <span style={{ fontSize:9,fontWeight:700,background:badge==='HOT'?'rgba(245,158,11,.2)':badge==='NEW'?'rgba(16,185,129,0.2)':'rgba(99,102,241,0.2)',color:badge==='HOT'?'#f59e0b':badge==='NEW'?'#10b981':'#818cf8',padding:'2px 6px',borderRadius:6,letterSpacing:'0.05em' }}>{badge}</span>}
                </Link>
              );
            })}
          </div>
