...
import { LayoutDashboard, Zap, Search, Radio, Bot, BarChart2, TrendingUp, Settings } from 'lucide-react';
    ...
    <aside style={{
        ...
        {NAV.map(({ href, label, icon: Icon, badge }) => {
          ...
          return (
            <Link key={href} href={href} style={{
              ...
              {badge && <span style={{ fontSize: 9, fontWeight: 700, background: 'rgba(99,102,241,0.2)', color: '#818cf8', padding: '2px 6px', borderRadius: 6, letterSpacing: '0.05em' }}>{badge}</span>}
            </Link>
          );
        })}
      </nav>
      ...
      <div style={{ padding: '12px 16px 16px', borderTop: '1px solid #1e2130' }}>
        ...
        <button
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', color: '#6b7194', fontSize: 12, cursor: 'pointer', padding: 0 }}
        >
          <span>→</span> Sign out
        </button>
      </div>
