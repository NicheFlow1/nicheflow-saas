    ...
    <div style={{maxWidth:720,margin:'0 auto'}}>
      <div style={{background:'linear-gradient(135deg,rgba(99,102,241,.1),rgba(139,92,246,.06))',border:'1px solid rgba(99,102,241,.25)',borderRadius:'var(--radius-2xl)',padding:24,marginBottom:14}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,marginBottom:14}}>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:800,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:6,display:'flex',alignItems:'center',gap:6}}><Zap size={10}/>Starter Kit &#183; {kit.keyword}</div>
...
')}/>
      ...
      <div style={{background:'linear-gradient(135deg,rgba(34,197,94,.06),rgba(34,197,94,.02))',border:'1px solid rgba(34,197,94,.2)',borderRadius:'var(--radius-2xl)',padding:20,marginBottom:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:12}}>
          <div style={{padding:'10px 12px',background:'rgba(0,0,0,.2)',borderRadius:'var(--radius-md)'}}><div style={{fontSize:9,color:'var(--text-disabled)',textTransform:'uppercase',marginBottom:3}}>Price Point</div><div style={{fontSize:12,fontWeight:700,color:'var(--text-primary)'}}>{kit.price_point}</div></div>
          <div style={{padding:'10px 12px',background:'rgba(0,0,0,.2)',borderRadius:'var(--radius-md)'}}><div style={{fontSize:9,color:'var(--text-disabled)',textTransform:'uppercase',marginBottom:3}}>30-Day Estimate</div><div style={{fontSize:12,fontWeight:700,color:'var(--success)'}}>{kit.revenue_30d_estimate}</div></div>
        </div>
...
function StarterContent(){
  ...
  if(step==='validating'||step==='building')return(
    <div style={{textAlign:'center',padding:'80px 24px'}}>
      ...
      <h2 style={{fontSize:'1.1rem',fontWeight:800,marginBottom:8}}>{step==='validating'?'Fetching real trend data...':'Building your complete starter kit...'}</h2>
      <p style={{fontSize:12,color:'var(--text-tertiary)',marginBottom:24}}>{step==='validating'?'Checking Google Trends for "'+keyword+'"':'Generating product ideas, landing page, Reddit communities, content hooks...'}</p>
      <div style={{display:'flex',justifyContent:'center',gap:6,flexWrap:'wrap'}}>
        {['Trend data','Opportunity','Product ideas','Landing page','Distribution'].map((s,i)=>(
          <div key={i} style={{padding:'4px 8px',borderRadius:4,fontSize:9,fontWeight:700,background:'rgba(99,102,241,.08)',color:'var(--brand-purple)',border:'1px solid rgba(99,102,241,.15)',opacity:step==='building'?1:i<2?1:.3}}>{s}</div>
        ))}
      </div>
    </div>
  )
