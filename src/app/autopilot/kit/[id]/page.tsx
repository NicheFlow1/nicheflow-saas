...
function CopyBtn({text}:{text:string}){
  ...
  return<button onClick={()=>{navigator.clipboard.writeText(text);setCopied(true);setTimeout(()=>setCopied(false),2000)}} style={{background:'none',border:'none',cursor:'pointer',color:copied?'var(--success)':'var(--text-disabled)',padding:'2px 6px',borderRadius:4,fontSize:10,display:'flex',alignItems:'center',gap:3}}>{copied?<CheckCircle size={11}/>:<Copy size={11}/>}{copied?'Copied':'Copy'}</button>
}
...
export default function KitPage({params}:{params:{id:string}}){
  ...
  useEffect(()=>{
    supabase.from('starter_kits').select('*').eq('id',params.id).single().then(({data})=>{setKit(data);setLoading(false)})
  },[params.id])
...
')}/>
          ...
          <div style={{background:'linear-gradient(135deg,#0a0a14,#111128)',border:'1px solid rgba(99,102,241,.2)',borderRadius:'var(--radius-xl)',padding:24,textAlign:'center'}}>
            ...
            {bullets.length>0&&(
              <div style={{textAlign:'left',maxWidth:300,margin:'0 auto 14px'}}>
          ...
          {hooks.map((h:any,i:number)=>(
            <div key={i} style={{padding:'10px 14px',background:'var(--bg-overlay)',borderLeft:'3px solid var(--brand-purple)',borderRadius:'0 var(--radius-md) var(--radius-md) 0',marginBottom:8}}>
              <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:12,fontWeight:600,color:'var(--text-primary)',marginBottom:3,lineHeight:1.5}}>&#x201C;{h.hook}&#x201D;</div>
