...
function CopyBtn({text}:{text:string}){
  ...
  return<button onClick={()=>{navigator.clipboard.writeText(text);setC(true);setTimeout(()=>setC(false),2000)}} style={{background:'none',border:'none',cursor:'pointer',color:c?'var(--success)':'var(--text-disabled)',padding:'2px 6px',borderRadius:4,fontSize:10,display:'flex',alignItems:'center',gap:3}}>{c?<CheckCircle size={11}/>:<Copy size={11}/>}{c?'Copied':'Copy'}</button>
}
...
export default function KitPage(){
  ...
  if(!kit)return<div style={{textAlign:'center',padding:60}}><p style={{color:'var(--text-tertiary)'}}>Kit not found</p><Link href='/autopilot' className='btn btn-ghost' style={{marginTop:16}}>Back</Link></div>
    ...
    <div style={{maxWidth:720,margin:'0 auto'}}>
      ...
      <div style={{background:'linear-gradient(135deg,rgba(99,102,241,.1),rgba(139,92,246,.06))',border:'1px solid rgba(99,102,241,.25)',borderRadius:'var(--radius-2xl)',padding:24,marginBottom:14}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16}}>
          <div style={{flex:1}}>
            <div style={{fontSize:9,fontWeight:800,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:6,display:'flex',alignItems:'center',gap:6}}><Zap size={10}/>{kit.keyword}</div>
            <h1 style={{fontSize:'1.25rem',fontWeight:900,color:'var(--text-primary)',marginBottom:8,lineHeight:1.3}}>{kit.one_liner}</h1>
            <p style={{fontSize:12,color:'var(--text-secondary)',lineHeight:1.7,marginBottom:12}}>{kit.problem_statement}</p>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              <div style={{padding:'8px 10px',background:'var(--bg-overlay)',borderRadius:'var(--radius-md)'}}><div style={{fontSize:9,color:'var(--text-disabled)',textTransform:'uppercase',marginBottom:2}}>Target</div><div style={{fontSize:11,color:'var(--text-secondary)',lineHeight:1.4}}>{kit.target_customer}</div></div>
              <div style={{padding:'8px 10px',background:'var(--bg-overlay)',borderRadius:'var(--radius-md)'}}><div style={{fontSize:9,color:'var(--text-disabled)',textTransform:'uppercase',marginBottom:2}}>Angle</div><div style={{fontSize:11,color:'var(--text-secondary)',lineHeight:1.4}}>{kit.unique_angle}</div></div>
            </div>
          </div>
      ...
      {actions.length>0&&<div style={{background:'var(--bg-elevated)',border:'1px solid var(--border-base)',borderRadius:'var(--radius-2xl)',padding:20,marginBottom:12}}>
        <div style={{fontSize:10,fontWeight:800,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:14}}>&#x26A1; First 7 Days</div>
...
')}/>
        ...
        <div style={{background:'linear-gradient(135deg,#0a0a14,#111128)',border:'1px solid rgba(99,102,241,.2)',borderRadius:'var(--radius-xl)',padding:24,textAlign:'center'}}>
          <div style={{fontSize:'1.3rem',fontWeight:900,color:'white',marginBottom:8,lineHeight:1.3}}>{kit.headline}</div>
          <div style={{fontSize:13,color:'rgba(255,255,255,.65)',marginBottom:14}}>{kit.subheadline}</div>
          {bullets.length>0&&<div style={{textAlign:'left',maxWidth:300,margin:'0 auto 14px'}}>
      ...
      {hooks.length>0&&<div style={{background:'var(--bg-elevated)',border:'1px solid var(--border-base)',borderRadius:'var(--radius-2xl)',padding:20,marginBottom:12}}>
        ...
        {hooks.map((h,i)=><div key={i} style={{padding:'10px 14px',background:'var(--bg-overlay)',borderLeft:'3px solid var(--brand-purple)',borderRadius:'0 var(--radius-md) var(--radius-md) 0',marginBottom:8}}>
          <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:8}}>
            <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:'var(--text-primary)',marginBottom:3,lineHeight:1.5}}>&#x201C;{h.hook}&#x201D;</div><span style={{fontSize:9,fontWeight:700,padding:'1px 5px',borderRadius:3,background:'rgba(99,102,241,.08)',color:'var(--brand-purple)',textTransform:'uppercase'}}>{h.platform}</span></div>
            <CopyBtn text={h.hook}/>
          </div>
        </div>)}
      </div>}
      ...
      <div style={{background:'linear-gradient(135deg,rgba(34,197,94,.06),rgba(34,197,94,.02))',border:'1px solid rgba(34,197,94,.2)',borderRadius:'var(--radius-2xl)',padding:20,marginBottom:16}}>
        <div style={{fontSize:10,fontWeight:800,color:'var(--success)',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:12}}>&#x1F4B0; Revenue Path</div>
