...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      style={{ background: 'none', border: 'none', cursor: 'pointer', color: copied ? 'var(--success)' : 'var(--text-disabled)', padding: '2px 6px', borderRadius: 4, fontSize: 10, display: 'flex', alignItems: 'center', gap: 3 }}
    >
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              <div style={{ padding: '8px 10px', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-md)' }}>
                <div style={{ fontSize: 9, color: 'var(--text-disabled)', textTransform: 'uppercase', marginBottom: 2 }}>Target</div>
                <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.4 }}>{kit.target_customer}</div>
              </div>
              <div style={{ padding: '8px 10px', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-md)' }}>
                <div style={{ fontSize: 9, color: 'var(--text-disabled)', textTransform: 'uppercase', marginBottom: 2 }}>Angle</div>
                <div style={{ fontSize: 11, color: 'var(--text-secondary)', lineHeight: 1.4 }}>{kit.unique_angle}</div>
              </div>
            </div>
          </div>
        ...
        <div style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-base)', borderRadius: 'var(--radius-2xl)', padding: 20, marginBottom: 12 }}>
          ...
          {actions.map((a: any, i: number) => (
            <div key={i} style={{ padding: '12px 14px', background: 'var(--bg-overlay)', borderRadius: 'var(--radius-lg)', marginBottom: 8, borderLeft: '3px solid var(--brand-purple)' }}>
              ...
              {a.expected_result && (
                <div style={{ fontSize: 10, color: 'var(--success)', marginTop: 4 }}>&#x2713; {3áa.expected_result}</div>
              )}
            </div>
          ))}
        </div>
