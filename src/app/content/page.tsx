...
export default function ContentPage() {
  ...
  async function loadLibrary(token: string) {
    try {
      const r = await fetch(`${SB_URL}/rest/v1/content_library?select=*&order=created_at.desc&limit=20`, { headers: { apikey: SB_ANON, Authorization: `Bearer ${token}` } });
  ...
  async function deleteItem(id: string) {
    if (!session) return;
    await fetch(`${SB_URL}/rest/v1/content_library?id=eq.${id}`, { method: 'DELETE', headers: { apikey: SB_ANON, Authorization: `Bearer ${session.access_token}`, Prefer: 'return=minimal' } });
    ...
    <div style={{ maxWidth: 780, margin: '0 auto', padding: '32px 24px' }}>
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border-base)', borderRadius: 14, padding: 22, marginBottom: 24 }}>
        <div style={{ marginBottom: 14 }}>
          ...
          <input value={keyword} onChange={e => setKeyword(e.target.value)} onKeyDown={e => e.key === 'Enter' && generate()} placeholder="e.g. longevity supplements, AI tools for founders..." style={{ width: '100%', background: 'var(--bg-base)', border: '1px solid var(--border-base)', borderRadius: 8, padding: '9px 13px', color: 'var(--text-primary)', fontSize: 14, outline: 'none', boxSizing: 'border-box' as const }} />
        </div>
        <div style={{ display: 'flex', gap: 14, marginBottom: 16, flexWrap: 'wrap' as const }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>{PLATFORMS.map(p => <button key={p} onClick={() => setPlatform(p)} style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: platform === p ? 'none' : '1px solid var(--border-base)', background: platform === p ? 'var(--brand-purple)' : 'transparent', color: platform === p ? '#fff' : 'var(--text-muted)' }}>{p}</button>)}</div>
          </div>
          <div>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>{TYPES.map(t => <button key={t} onClick={() => setType(t)} style={{ padding: '5px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, cursor: 'pointer', border: type === t ? 'none' : '1px solid var(--border-base)', background: type === t ? 'rgba(139,92,246,.8)' : 'transparent', color: type === t ? '#fff' : 'var(--text-muted)' }}>{t}</button>)}</div>
          </div>
        </div>
        {error && <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 13px', background: 'rgba(239,68,68,.08)', border: '1px solid rgba(239,68,68,.2)', borderRadius: 8, marginBottom: 12 }}><AlertCircle size={14} color="#ef4444" /><span style={{ fontSize: 13, color: '#ef4444' }}>{error}</span></div>}
        <button onClick={generate} disabled={loading || !keyword.trim()} style={{ width: '100%', background: loading || !keyword.trim() ? 'var(--bg-hover)' : 'var(--brand-purple)', color: loading || !keyword.trim() ? 'var(--text-muted)' : '#fff', border: 'none', padding: '11px', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: loading || !keyword.trim() ? 'not-allowed' : 'pointer' }}>
      ...
      {library.length > 0 && (
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {library.map(item => (
              <div key={item.id || item.created_at} style={{ background: 'var(--bg-card)', border: '1px solid var(--border-base)', borderRadius: 12, padding: 16 }}>
      ...
      {library.length === 0 && !loading && <div style={{ textAlign: 'center', padding: '40px 24px', border: '1px dashed var(--border-base)', borderRadius: 12 }}><p style={{ color: 'var(--text-muted)', fontSize: 13 }}>No content yet. Generate your first piece above.</p></div>}
    </div>
  );
}
