...
export default function TrendingPage() {
  ...
  async function scan() {
    ...
    try {
      const data = await callApi({ action: "trending", category });
  ...
  async function saveToWatchlist(t: any, i: number) {
    ...
    try { await callApi({ action: "watchlist_add", niche: t.topic, score: t.score, signal: t.signal, category: t.category }); setSaved(p => ({ ...p, [i]: true })); } catch {}
  ...
  async function getForecast(t: any, i: number) {
    ...
    try { const data = await callApi({ action: "forecast", topic: t.topic }); setForecasts(p => ({ ...p, [i]: data })); } catch {}
    setForecasting(p => ({ ...p, [i]: false }));
  }

  const spinnerStyle: React.CSSProperties = { width: 16, height: 16, border: "2px solid rgba(255,255,255,0.25)", borderTopColor: "white", borderRadius: "50%", animation: "spin 0.8s linear infinite" };
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg-base)" }}>
      <div style={{ borderBottom: "1px solid var(--border)", background: "var(--bg-surface)", padding: "1.75rem 2.5rem 1.5rem" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          ...
          <p style={{ color: "var(--text-muted)", margin: "0 0 1.25rem", fontSize: "0.9rem" }}>Discover what is blowing up right now — before it goes mainstream.</p>
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", padding: "1.75rem 2.5rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "1rem", marginBottom: "1.75rem" }}>
          <button onClick={scan} disabled={loading} style={{ background: loading ? "var(--bg-elevated)" : "var(--accent)", color: "white", border: "none", borderRadius: "var(--radius)", padding: "0.7rem 1.75rem", fontWeight: 700, fontSize: "0.95rem", display: "inline-flex", alignItems: "center", gap: "0.55rem", cursor: loading ? "not-allowed" : "pointer", transition: "all 0.2s" }}>
            {loading ? <><div style={spinnerStyle} /> Scanning markets...</> : "Scan Trending Topics"}
          </button>
        ...
        {!loading && topics.length === 0 && !error && (
          <div style={{ textAlign: "center", padding: "5rem 2rem", background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)" }}>
            <div style={{ width: 56, height: 56, background: "var(--bg-elevated)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 1rem", fontSize: "1.5rem" }}>fire</div>
            <h3 style={{ color: "var(--text-primary)", fontWeight: 700, marginBottom: "0.4rem" }}>No trends loaded</h3>
            <p style={{ color: "var(--text-muted)", fontSize: "0.875rem" }}>Pick a category and hit Scan Trending Topics</p>
          </div>
        )}
        ...
        {topics.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(460px, 1fr))", gap: "1.25rem" }}>
            {topics.map((t, i) => {
              ...
              return (
                <div key={i} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", overflow: "hidden", transition: "all 0.2s" }}
                ...
                >
                  ...
                  <div style={{ padding: "1.25rem" }}>
                    ...
                    {fc && (
                      <div style={{ background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)", borderRadius: "var(--radius-sm)", padding: "0.85rem", marginBottom: "1rem" }}>
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "0.5rem", marginBottom: "0.6rem" }}>
                          {([{label:"3 months",d:fc.forecast_3m},{label:"6 months",d:fc.forecast_6m},{label:"12 months",d:fc.forecast_12m}] as any[]).map((f, j) => f.d && (
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", marginBottom: "0.5rem" }}>
                      <button onClick={() => window.location.href = "/dashboard/starter?keyword=" + encodeURIComponent(t.topic)} style={{ background: "var(--accent)", color: "white", border: "none", borderRadius: "var(--radius-sm)", padding: "0.6rem", fontWeight: 700, fontSize: "0.82rem", cursor: "pointer" }}>Build Starter Kit</button>
                      <button onClick={() => window.location.href = "/dashboard/validate?q=" + encodeURIComponent(t.topic)} style={{ background: "transparent", color: "var(--text-secondary)", border: "1px solid var(--border)", borderRadius: "var(--radius-sm)", padding: "0.6rem", fontWeight: 600, fontSize: "0.82rem", cursor: "pointer" }}>Validate Trend</button>
                    </div>
