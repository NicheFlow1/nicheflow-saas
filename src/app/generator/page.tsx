...
const MODULES = [
  ...
  { key:'product', label:'Product Builder', icon:'🛠️', desc:'Designing MVP...' },
...
export default function GeneratorPage() {
  ...
  const [state, setState] = useState({ projectId: null as string|null, niche:'', projectName:'', outputs:{} as Record<string,unknown>, step:0, running:false, error:null as string|null })
  async function startGeneration() {
    ...
    for (let i = 0; i < MODULES.length; i++) {
      ...
      if (mod.key==='idea' && gd.output?.businessName) projectName = gd.output.businessName as string
    ...
    <div className="max-w-2xl mx-auto space-y-6">
        ...
        {MODULES.map((m,i) => {
          ...
          return (
            <div key={m.key} className={cn('flex items-center gap-3 p-3 rounded-xl', running?'bg-nf-purple/10 border border-nf-purple/30':done?'bg-nf-surface2':'opacity-40')}>
              <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center', running?'bg-nf-purple/20':done?'bg-nf-teal/10':'bg-nf-surface3')}>
                {done?'✓':running?<Loader2 size={14} className="animate-spin text-nf-purple"/>:m.icon}
              </div>
              <div>
                <div className="text-sm font-semibold">{m.label}</div>
                <div className="text-xs text-muted-foreground">{done?'Complete':running?m.desc:'Queued'}</div>
              </div>
              {done && <CheckCircle size={14} className="text-nf-teal ml-auto"/>}
            </div>
          )
        })}
      </div>
      {isDone && (
        <div className="flex gap-3">
          ...
          <button onClick={() => { setState({projectId:null,niche:'',projectName:'',outputs:{},step:0,running:false,error:null}); setNiche('') }} className="nf-btn-ghost border border-nf-border px-5 py-2.5 rounded-xl">Generate Another</button>
        </div>
      )}
    </div>
  )
