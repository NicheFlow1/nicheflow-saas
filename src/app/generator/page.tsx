...
import{Zap,TrendingUp,DollarSign,AlertTriangle,CheckCircle,XCircle,Clock,Sparkles,ChevronDown,ChevronUp,BookmarkPlus,Shield,Rocket,Crosshair}from'lucide-react'
...
const MODES=[
  ...
  {id:'compete',label:'Competition X-Ray',desc:'Map the battlefield'}
]
const PLACEHOLDERS:Record<string,string>={
  analyze:'Enter a niche, market, or idea to analyze... e.g. AI tools for freelance designers, eco pet supplements',
  discover:'Enter a broad space to explore 3 hidden niches... e.g. health and wellness, creator economy, fintech',
  trend:'Enter a trend or keyword to analyze its lifecycle... e.g. AI agents, quiet luxury, longevity supplements',
  compete:'Enter a market to X-Ray the competition... e.g. project management SaaS, online fitness coaching'
}
...
function Bar({label,score,color='bg-indigo-500'}:{label:string,score:number,color?:string}){
  return(
    <div className='space-y-1'>
      ...
      <div className='h-1 bg-[#0f0f1e] rounded-full overflow-hidden'>
  ...
  return(
    <div className='space-y-3'>
        ...
        <div className='flex items-start justify-between gap-4 mb-4'>
          <div className='flex-1'>
            <div className='flex items-center gap-2 mb-2 flex-wrap'>
              ...
              <span className='text-[10px] font-mono px-2 py-1 rounded bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 capitalize'>{opp.lifecycle_stage||'emerging'}</span>
          ...
          <div className='flex-shrink-0 text-center'>
            <ScoreRing score={sc}/>
            <div className='text-[9px] text-gray-600 font-mono mt-1'>SCORE</div>
          </div>
        </div>
        <div className='grid grid-cols-4 gap-2 mb-4'>
          {metrics.map(([l,s,c])=><Bar key={l} label={l} score={s} color={c}/>)}
        </div>
          ...
          {opp.trend_signals.slice(0,5).map((s:any,i:number)=>(
            <div key={i} className='flex items-center gap-3 p-3 bg-[#0f0f1e] rounded-xl'>
              <div className='w-20 flex-shrink-0'>
                <div className='text-[9px] font-mono text-indigo-400 uppercase mb-1'>{s.platform}</div>
                <div className='h-1 bg-[#1a1a2e] rounded-full overflow-hidden'><div className='h-full bg-indigo-500 rounded-full' style={{width:(s.strength||0)+'%'}}/></div>
              </div>
        ...
        <Sec title='Pain Points' Icon={AlertTriangle}>
          {opp.pain_points.slice(0,4).map((p:any,i:number)=>(
            <div key={i} className='flex items-start gap-3 p-3 bg-[#0f0f1e] rounded-xl'>
              <span className={'text-[9px] font-mono px-2 py-1 rounded flex-shrink-0 '+(p.intensity==='critical'?'bg-red-500/20 text-red-400':p.intensity==='high'?'bg-orange-500/20 text-orange-400':p.intensity==='medium'?'bg-amber-500/20 text-amber-400':'bg-gray-500/20 text-gray-400')}>
                {(p.intensity||'med').toUpperCase()}
              </span>
              <div>
                <p className='text-xs text-gray-200 font-medium'>{p.pain}</p>
                {p.evidence&&<p className='text-[10px] text-gray-500 mt-0.5'>{p.evidence}</p>}
              </div>
            </div>
          ))}
        </Sec>
        ...
        <Sec title='Competition X-Ray' Icon={Shield}>
          {(map.white_spaces||[]).length>0&&(
            <div className='p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-xl'>
              <div className='text-[9px] font-mono text-emerald-400 mb-2'>WHITE SPACES</div>
              {map.white_spaces.map((w:string,i:number)=>(
                <div key={i} className='flex items-center gap-2 text-xs text-gray-300 mb-1'>
                  <div className='w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0'/>{w}
                </div>
              ))}
            </div>
          )}
          {(map.incumbents||[]).slice(0,3).map((c:any,i:number)=>(
            <div key={i} className='p-3 bg-[#0f0f1e] rounded-xl'>
              <div className='flex justify-between mb-1'>
                ...
                {c.revenue&&<span className='text-[10px] text-gray-500'>{c.revenue}</span>}
              </div>
        ...
        <Sec title='Monetization Intelligence' Icon={DollarSign}>
          ...
          {(mono.paths||[]).length>0&&(
            <div className='grid grid-cols-2 gap-2'>
              {mono.paths.slice(0,4).map((p:any,i:number)=>(
                <div key={i} className='p-3 bg-[#0f0f1e] rounded-xl'>
                  <div className='text-[9px] font-mono text-gray-500 mb-1'>{p.model}</div>
                  <div className='text-sm font-bold text-amber-400'>{p.price}</div>
                  {p.margin&&<div className='text-[10px] text-gray-500'>{p.margin}</div>}
                  <div className='text-[10px] text-indigo-400 mt-1'>{p.timeline}</div>
                </div>
              ))}
            </div>
          )}
          {mono.ltv_estimate&&(
            <div className='grid grid-cols-2 gap-2'>
              <div className='p-3 bg-[#0f0f1e] rounded-xl'><div className='text-[9px] font-mono text-gray-500 mb-1'>LTV</div><div className='text-sm font-bold text-emerald-400'>{mono.ltv_estimate}</div></div>
              <div className='p-3 bg-[#0f0f1e] rounded-xl'><div className='text-[9px] font-mono text-gray-500 mb-1'>CAC</div><div className='text-sm font-bold text-amber-400'>{mono.cac_estimate||'TBD'}</div></div>
            </div>
          )}
        </Sec>
      )}
      ...
      {exec.mvp_definition&&(
        <Sec title='Execution Pipeline' Icon={Rocket}>
          <div className='p-3 bg-indigo-500/5 border border-indigo-500/20 rounded-xl'>
            <div className='text-[9px] font-mono text-indigo-400 mb-1'>MVP</div>
            <p className='text-xs text-gray-200'>{exec.mvp_definition}</p>
          </div>
          {exec.first_dollar&&(
            <div className='p-3 bg-emerald-500/5 border border-emerald-500/20 rounded-xl'>
              <div className='text-[9px] font-mono text-emerald-400 mb-1'>FIRST REVENUE IN 30 DAYS</div>
              <p className='text-xs text-gray-200'>{exec.first_dollar}</p>
            </div>
          )}
              ...
              return(
                <div key={k} className='flex gap-3'>
                  <div className='flex flex-col items-center flex-shrink-0'>
                    ...
                    {idx<3&&<div className='w-px flex-1 bg-[#1a1a2e] my-1'/>}
                  </div>
                  <div className='pb-3 flex-1'>
                    <div className='text-[9px] font-mono text-indigo-400 mb-0.5'>
                      {label}{step.cost&&<span className='text-gray-600 ml-1'>{step.cost}</span>}
                    </div>
                    <p className='text-xs text-gray-200 font-medium'>{step.action}</p>
                    {step.outcome&&<p className='text-[10px] text-gray-500 mt-0.5'>{step.outcome}</p>}
                  </div>
                </div>
              )
            })}
      ...
      {ai.unique_insight&&(
        <div className='bg-gradient-to-r from-indigo-500/5 to-purple-500/5 border border-indigo-500/20 rounded-2xl p-4'>
          <div className='flex items-center gap-2 mb-2'>
            ...
            <span className='text-[9px] font-mono text-indigo-300 uppercase tracking-wider'>Key Intelligence Insight</span>
          </div>
      ...
      <button
        ...
        disabled={saved}
        className='w-full flex items-center justify-center gap-2 py-3 rounded-xl border border-[#1a1a2e] text-sm text-gray-500 hover:bg-[#0f0f1e] hover:text-indigo-300 hover:border-indigo-500/30 transition-all disabled:opacity-50'
      >
  ...
  return(
    <div className='max-w-3xl mx-auto space-y-4'>
      <div className='flex items-start justify-between'>
        <div>
          <h1 className='text-2xl font-black text-white'>Market Intelligence Engine</h1>
          <p className='text-sm text-gray-500 mt-0.5'>Find opportunities before they become obvious</p>
        </div>
        <div className='text-right'>
          <div className='text-[10px] font-mono text-gray-600 mb-1'>{used}/{limit} CREDITS</div>
          <div className='w-24 h-1 bg-[#0f0f1e] rounded-full overflow-hidden'>
      ...
      <div className='bg-[#0a0a14] border border-[#1a1a2e] rounded-2xl p-4'>
        <form onSubmit={run} className='space-y-3'>
          <textarea
            ...
            rows={3}
            className='w-full bg-[#0f0f1e] border border-[#1a1a2e] rounded-xl px-4 py-3 text-sm text-gray-200 placeholder-gray-600 focus:outline-none focus:border-indigo-500/50 resize-none'
          />
          {error&&(
          ...
          <button
            ...
            className='w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-bold text-sm disabled:opacity-40 hover:opacity-90 transition-all'
          >
            {loading
              ?<><div className='w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin'/>Analyzing Intelligence...</>
              :<><Zap size={15}/>Run Intelligence Engine</>
            }
          </button>
          {used>=limit&&(
            <p className='text-[10px] text-center text-gray-600'>Credits exhausted. <a href='/settings/billing' className='text-indigo-400 hover:underline'>Upgrade plan</a></p>
          )}
        </form>
      </div>
