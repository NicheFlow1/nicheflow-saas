...
import{TrendingUp,AlertTriangle,CheckCircle,XCircle,Clock,Sparkles,Search,Database,ArrowUpRight,ArrowDownRight,Minus,RefreshCw,BookmarkPlus}from'lucide-react'
...
function TrendChart({data}:{data:Array<{date:string,value:number}>}){
  ...
  // Show every ~52nd point as year label (approx yearly)
  const yearLabels=data.filter((_,i)=>i%(Math.floor(data.length/5)||1)===0||i===data.length-1)
  return(
    <div style={{width:'100%',overflowX:'auto'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',marginTop:4,paddingLeft:pad,paddingRight:pad}}>
        {yearLabels.slice(0,5).map((d,i)=>(
          <span key={i} style={{fontSize:9,color:'var(--text-disabled)',fontFamily:'monospace'}}>{d.date.slice(0,7)}</span>
        ))}
      </div>
    </div>
  )
...
function EvidenceScore({label,score,basis,color='var(--brand-purple)'}:{label:string,score:number,basis:string,color?:string}){
  const[showBasis,setShowBasis]=useState(false)
  const barColor=score>=70?'var(--success)':score>=45?'var(--warning)':'var(--danger)'
  return(
    <div style={{marginBottom:12}}>
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:5}}>
        <div style={{display:'flex',alignItems:'center',gap:6}}>
          <span style={{fontSize:11,fontWeight:700,color:'var(--text-primary)'}}>{label}</span>
          {score>0&&<button onClick={()=>setShowBasis(s=>!s)} style={{fontSize:9,color:'var(--brand-purple)',background:'rgba(99,102,241,0.08)',border:'1px solid rgba(99,102,241,0.2)',borderRadius:3,padding:'1px 5px',cursor:'pointer',fontFamily:'monospace'}}>WHY</button>}
        </div>
        {score>0?<span style={{fontSize:13,fontWeight:900,color:barColor}}>{score}</span>:<span style={{fontSize:10,color:'var(--text-disabled)'}}>N/A</span>}
      </div>
      {score>0&&<div style={{height:4,background:'var(--bg-subtle)',borderRadius:99,overflow:'hidden'}}><div style={{height:'100%',borderRadius:99,background:barColor,width:score+'%',transition:'width 1s cubic-bezier(0.4,0,0.2,1)'}}/></div>}
      {showBasis&&<div style={{marginTop:6,padding:'8px 10px',background:'rgba(99,102,241,0.04)',border:'1px solid rgba(99,102,241,0.12)',borderRadius:6,fontSize:10,color:'var(--text-secondary)',lineHeight:1.6}}>{basis}</div>}
    </div>
  )
}
  ...
  return(
    <div style={{maxWidth:700,margin:'0 auto'}}>
      <div className='page-header' style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
        <div>
          <h1>Trend Validation</h1>
          <p>Real Google Trends data — AI interprets what it means</p>
        </div>
        <div style={{textAlign:'right'}}>
          ...
          <div style={{width:80,height:3,background:'var(--bg-subtle)',borderRadius:99,overflow:'hidden'}}>
            <div style={{height:'100%',background:'linear-gradient(90deg,var(--brand-purple),var(--brand-pink))',borderRadius:99,width:Math.min((used/limit)*100,100)+'%',transition:'width 0.5s'}}/>
          </div>
        </div>
      </div>
      ...
      <div className='card' style={{padding:18,marginBottom:20}}>
        <form onSubmit={run}>
          <div style={{display:'flex',gap:8,marginBottom:12}}>
            <div style={{flex:1,position:'relative'}}>
              ...
              <input value={keyword} onChange={e=>setKeyword(e.target.value)} placeholder='Enter a market or keyword to validate... e.g. longevity supplements' className='input' style={{paddingLeft:34}}/>
            </div>
            <button type='submit' disabled={loading||!keyword.trim()||used>=limit} className='btn btn-primary' style={{whiteSpace:'nowrap'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,marginTop:10,padding:'6px 10px',background:'var(--bg-overlay)',borderRadius:6}}>
          ...
          <span style={{fontSize:10,color:'var(--text-tertiary)'}}>Data source: Google Trends via SerpAPI. Scores calculated from real search volume data — not AI-generated numbers.</span>
        </div>
      </div>
      ...
      {report&&(
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div className='card' style={{padding:22}}>
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:16,marginBottom:18}}>
              <div style={{flex:1}}>
                ...
                <h2 style={{fontSize:'1.25rem',fontWeight:900,color:'var(--text-primary)',marginBottom:4}}>{report.keyword}</h2>
            ...
            {report.ai_interpretation&&(
              <div className='insight-box' style={{marginBottom:16}}>
                <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:8}}>
                  ...
                  <span style={{fontSize:9,fontWeight:700,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'0.06em'}}>AI Interpretation of Trend Data</span>
                </div>
            ...
            <div style={{display:'flex',gap:8,marginTop:4}}>
              <button onClick={saveToRadar} disabled={saved} className='btn btn-ghost btn-sm' style={{flex:1,justifyContent:'center'}}>
          ...
          <div className='card' style={{padding:18}}>
            ...
            <EvidenceScore label='Competition' score={report.competition_score||0} basis={report.competition_score_basis||'AI estimate only'}/>
          </div>
          ...
          {(report.top_queries?.length>0||report.rising_queries?.length>0)&&(
            <div className='card' style={{padding:18}}>
              <div style={{fontSize:10,fontWeight:700,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:14,display:'flex',alignItems:'center',gap:6}}>
                <Search size={11}/>Related Search Queries <span style={{fontSize:9,padding:'1px 6px',borderRadius:3,background:'rgba(34,197,94,0.08)',color:'var(--success)',border:'1px solid rgba(34,197,94,0.15)',fontFamily:'monospace',fontWeight:400,textTransform:'none',letterSpacing:0}}>REAL DATA</span>
              </div>
                  ...
                  <div>
                    ...
                    {report.top_queries.slice(0,8).map((q:any,i:number)=>(
                      <div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                        ...
                        <span style={{fontSize:10,fontWeight:700,color:'var(--brand-purple)',marginLeft:8,flexShrink:0}}>{q.value}</span>
                      </div>
                    ))}
                  </div>
                )}
                {report.rising_queries?.length>0&&(
                  <div>
                    ...
                    {report.rising_queries.slice(0,8).map((q:any,i:number)=>(
                      <div key={i} style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'5px 0',borderBottom:'1px solid rgba(255,255,255,0.03)'}}>
                        <span style={{fontSize:11,color:'var(--text-secondary)'}}>{q.query}</span>
                        <span style={{fontSize:9,fontWeight:700,color:'var(--success)',background:'rgba(34,197,94,0.08)',padding:'1px 5px',borderRadius:3,flexShrink:0}}>{String(q.value).includes('Breakout')?'BREAKOUT':String(q.value)}</span>
                      </div>
                    ))}
                  </div>
                )}
              </div>
              {report.ai_interpretation&&report.top_queries?.length>0&&(
                <div style={{marginTop:14,padding:'10px 12px',background:'var(--bg-overlay)',borderRadius:'var(--radius-md)',fontSize:11,color:'var(--text-tertiary)',lineHeight:1.6}}>
                  <span style={{fontWeight:700,color:'var(--brand-purple)'}}>AI — </span>
                  {report.ai_interpretation?.what_queries_reveal||'AI interpretation of queries not available.'}
                </div>
              )}
            </div>
          )}
          ...
          {report.ai_opportunities?.length>0&&(
            <div className='card' style={{padding:18}}>
              <div style={{fontSize:10,fontWeight:700,color:'var(--brand-purple)',textTransform:'uppercase',letterSpacing:'0.06em',marginBottom:14,display:'flex',alignItems:'center',gap:6}}>
                <Sparkles size={11}/>Opportunities Found
              </div>
              {report.ai_opportunities.map((o:any,i:number)=>(
                <div key={i} style={{padding:'12px 14px',background:'var(--bg-overlay)',borderRadius:'var(--radius-lg)',marginBottom:8}}>
                  <div style={{fontSize:12,fontWeight:700,color:'var(--text-primary)',marginBottom:4}}>{o.title}</div>
          ...
          {report.ai_action_plan&&Object.keys(report.ai_action_plan).length>0&&(
            <div className='card' style={{padding:18}}>
              ...
              {[['immediate','This Week'],['validation_step','Validation Step'],['first_revenue','First Revenue']].map(([k,label])=>(
